...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST)) { //on initialise nos messages d'erreurs;
    ...
    if (isset($_FILES['image']) and $_FILES['image']['error'] == UPLOAD_ERR_OK) {
        ...
        if (in_array($extension, $validExtensions)) {
            $imageName = date('d-m-y') . $name . '.' .  $extension;
    ...
    if ($valid) {
        ...
        $sql = "INSERT INTO user (name,firstname,age,tel, email, pays,comment, metier,url, image) values(?, ?, ?, ? , ? , ? , ? , ?, ? , ?)";
        $q = $pdo->prepare($sql);
        $q->execute(array($name, $firstname, $age, $tel, $email, $pays, $comment, $metier, $url, $imageName));
        Database::disconnect();
        ...
        <form method="post" action="add.php" enctype="multipart/form-data">
            ...
            <div class="control-group  <?php echo !empty($urlError) ? 'error' : ''; ?>">
                ...
                <div class="control-group <?php echo !empty($imgError) ? 'error' : ''; ?>">
                    ...
                    <div class="controls">
                        <input name="image" type="file" placeholder="Uploud your image" value="<?php echo !empty($image) ? $image : ''; ?>">
